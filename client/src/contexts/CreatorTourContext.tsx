...
const CREATOR_TOUR_STORAGE_KEY = "affiliatexchange_creator_tour_completed";
const CREATOR_TOUR_SEEN_PAGES_KEY = "affiliatexchange_creator_tour_seen_pages";
...
interface CreatorTourContextType {
  // Global tour state
...
const CreatorTourContext = createContext<CreatorTourContextType | undefined>(undefined);

export function CreatorTourProvider({ children }: { children: React.ReactNode }) {
  const [hasCompletedInitialTour, setHasCompletedInitialTour] = useState<boolean>(() => {
    try {
      return localStorage.getItem(CREATOR_TOUR_STORAGE_KEY) === "true";
    } catch {
      return false;
    }
  });
  ...
  const [seenPages, setSeenPages] = useState<string[]>(() => {
    try {
      const stored = localStorage.getItem(CREATOR_TOUR_SEEN_PAGES_KEY);
  ...
  const markPageTourSeen = useCallback((pageId: string) => {
    setSeenPages(prev => {
      ...
      try {
        localStorage.setItem(CREATOR_TOUR_SEEN_PAGES_KEY, JSON.stringify(updated));
      } catch {
    ...
    if (!hasCompletedInitialTour) {
      ...
      try {
        localStorage.setItem(CREATOR_TOUR_STORAGE_KEY, "true");
      } catch {
        // Ignore storage errors
      }
    }
  ...
  const resetAllTours = useCallback(() => {
    ...
    setCurrentPageSteps([]);
    try {
      localStorage.removeItem(CREATOR_TOUR_STORAGE_KEY);
      localStorage.removeItem(CREATOR_TOUR_SEEN_PAGES_KEY);
    } catch {
      // Ignore storage errors
    }
  }, []);
